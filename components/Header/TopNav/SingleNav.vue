<template>
  <div v-if="isDesktop" class="scrollactive-nav">
    <template v-if="singleNav">
      <v-btn v-for="(item, index) in dataMenu" :key="index" :href="item.link"
        :class="{ active: activeMenu === item.name }" class="menu-link" @click="scrollToMyEl(item.name)"
        v-text="$t('header.' + item.name)" />
    </template>
    <template v-if="!singleNav">
      <v-btn v-for="(item, index) in dataMenu" :key="index" :href="'/' + item.link" v-text="$t('header.' + item.name)" />
    </template>
  </div>
</template>
